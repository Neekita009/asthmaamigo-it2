...
// data for the map from csv file
	queue()
	.defer(d3.json, 'AustraliaGeo.json')
	// .defer(d3.csv, 'PredictedVictoria.csv')
	.await(makeMyMap);
...
bool = true;
	...
	.on('mouseover', function(d) {
		...
		if(d.properties.STATE_NAME != "Victoria")
		{
			 // tooltip.html(d.properties.STATE_NAME + ' Patient'+ ": "+ d.properties.Asthma+"<br><b>Click Me</b>")
        // .styles({
...
// Data for CsV file
Plotly.d3.csv('PredictedVictoria.csv', (err, rows) => {
  var data = Age.map(y => {
...
// creating graph 
Plotly.newPlot('containerLine', data,layout,{responsive: true, displayModeBar: false});
	})
	// Second graph
	CreateSecondGraph();
...
// data from csv file
Plotly.d3.csv('Asthma_BOD.csv', (err, rows) => {
...
// ploting the graph
  Plotly.newPlot('containerStackBar', data,layout,{responsive: true, displayModeBar: false});
})
...
// var el = document.getElementById("Vic");
